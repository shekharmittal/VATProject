...
merge 1:1 DealerTIN TaxQuarter using "F:\2a2b_analysis\IntraFirmAnalysis\quarterly_dealers_all.dta", keepusing(TransActionCount SumAmount SumTaxAmount SumTotalAmount)
...
preserve
drop if PurchaseRatio>100|PurchaseRatio<0.01&PurchaseRatio!=.
collapse (mean) PurchaseRatio (mean) TaxCreditBeforeAdjustment SumTaxAmount, by(TaxQuarter)
twoway (connected TaxCreditBeforeAdjustment TaxQuarter if PurchaseRatio!=.&TaxQuarter!=12) (connected SumTaxAmount TaxQuarter if PurchaseRatio!=.&TaxQuarter!=12, lpattern(dash)), graphregion(color(white)) legend(order(1 "Tax Credits (from consolidated)" 2 "Tax Credits (from transactions)")) note("For all firms. Numbers in million rupees") title("Tax Credits: Consolidated vs Transaction Data")
