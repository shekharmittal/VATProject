/*In this file we do the heterogeneity analysis*/
...
//Regression of the 1st decile
gen Treat2=1 if Treat==1&TreatGroup==1&TaxYear==1
replace Treat2=0 if Treat==0&ControlGroup==1&TaxYear==1
...
estimates store Decile1TaxCredit
...
estimates store Decile1OutputTax

areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile1Diff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile1lDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile1Interstate

//Regression of the 5th decile
drop Treat2 iPostTreat
gen Treat2=1 if Treat==1&TreatGroup==5&TaxYear==1
replace Treat2=0 if Treat==0&ControlGroup==5&TaxYear==1
...
areg PositiveContribution Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile5PositiveContribution
areg VatIncrease Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile5VatIncrease
areg MoneyDeposited Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile5MoneyDeposited
areg TaxCreditBeforeAdjustment Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile5TaxCredit
areg OutputTaxBeforeAdjustment  Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile5OutputTax

areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile5Diff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile5lDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile5Interstate



//Regression of the 6th decile
drop Treat2 iPostTreat
gen Treat2=1 if Treat==1&TreatGroup==6&TaxYear==1
replace Treat2=0 if Treat==0&ControlGroup==6&TaxYear==1
...
areg PositiveContribution Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile6PositiveContribution
areg VatIncrease Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile6VatIncrease
areg MoneyDeposited Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile6MoneyDeposited
areg TaxCreditBeforeAdjustment Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile6TaxCredit
areg OutputTaxBeforeAdjustment  Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile6OutputTax


areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile6Diff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile6lDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile6Interstate



//Regression of the 7th decile
drop Treat2 iPostTreat
gen Treat2=1 if Treat==1&TreatGroup==7&TaxYear==1
replace Treat2=0 if Treat==0&ControlGroup==7&TaxYear==1
...
areg PositiveContribution Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile7PositiveContribution
areg VatIncrease Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile7VatIncrease
areg MoneyDeposited Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile7MoneyDeposited
areg TaxCreditBeforeAdjustment Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile7TaxCredit
areg OutputTaxBeforeAdjustment  Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile7OutputTax

areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile7Diff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile7lDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile7Interstate

//Regression of the 8th decile
drop Treat2 iPostTreat
gen Treat2=1 if Treat==1&TreatGroup==8&TaxYear==1
replace Treat2=0 if Treat==0&ControlGroup==8&TaxYear==1
...
areg PositiveContribution Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile8PositiveContribution
areg VatIncrease Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile8VatIncrease
areg MoneyDeposited Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile8MoneyDeposited
areg TaxCreditBeforeAdjustment Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile8TaxCredit
areg OutputTaxBeforeAdjustment  Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile8OutputTax

areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile8Diff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile8lDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile8Interstate


//Regression of the 9th decile
drop Treat2 iPostTreat
gen Treat2=1 if Treat==1&TreatGroup==9&TaxYear==1
replace Treat2=0 if Treat==0&ControlGroup==9&TaxYear==1
...
areg PositiveContribution Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile9PositiveContribution
areg VatIncrease Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile9VatIncrease
areg MoneyDeposited Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile9MoneyDeposited
areg TaxCreditBeforeAdjustment Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile9TaxCredit
areg OutputTaxBeforeAdjustment  Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store Decile9OutputTax

areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile9Diff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile9lDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store Decile9Interstate

//Regression of the 10th decile
drop Treat2 iPostTreat
...
areg PositiveContribution Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store TopPositiveContribution
areg VatIncrease Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store TopVatIncrease
areg MoneyDeposited Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store TopMoneyDeposited
areg TaxCreditBeforeAdjustment Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store TopTaxCredit
areg OutputTaxBeforeAdjustment  Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5 if TotalCount==5, cluster(DealerTIN) absorb(DealerTIN)
estimates store TopOutputTax

areg Diff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store TopDiff

areg lDiff Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store ToplDiff


areg InterstateRatio Post iPostTreat iTaxYear2 iTaxYear4 iTaxYear5  if TotalCount==5, absorb(DealerTIN) cluster(DealerTIN)
estimates store TopInterstate

label variable iPostTreat "Post*Wholesaler"

#delimit ;
coefplot (Decile1PositiveContribution, label("1-4 Deciles")) (Decile5PositiveContribution, label("5th Decile")) 
         (Decile6PositiveContribution, label("6th Decile")) (Decile7PositiveContribution, label("7th Decile")) 
		 (Decile8PositiveContribution, label("8th Decile")) (Decile9PositiveContribution, label("9th Decile")) 
		 (TopPositiveContribution, label("10th Decile")) , drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
...
#delimit ;
coefplot (Decile1VatIncrease, label("1-4 Deciles")) (Decile5VatIncrease, label("5th Decile")) 
         (Decile6VatIncrease, label("6th Decile")) (Decile7VatIncrease, label("7th Decile")) 
		 (Decile8VatIncrease, label("8th Decile")) (Decile9VatIncrease, label("9th Decile")) 
		 (TopVatIncrease, label("10th Decile")) , drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
...
#delimit ;
coefplot (Decile1MoneyDeposited, label("1-4 Deciles")) (Decile5MoneyDeposited, label("5th Decile")) 
         (Decile6MoneyDeposited, label("6th Decile")) (Decile7MoneyDeposited, label("7th Decile")) 
		 (Decile8MoneyDeposited, label("8th Decile")) (Decile9MoneyDeposited, label("9th Decile")) 
		 (TopMoneyDeposited, label("10th Decile")) , drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
...
#delimit ;
coefplot (Decile1TaxCredit, label("1-4 Deciles")) (Decile5TaxCredit, label("5th Decile")) 
         (Decile6TaxCredit, label("6th Decile")) (Decile7TaxCredit, label("7th Decile")) 
		 (Decile8TaxCredit, label("8th Decile")) (Decile9TaxCredit, label("9th Decile")) 
		 (TopTaxCredit, label("10th Decile")) , drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
...
#delimit ;
coefplot (Decile1OutputTax, label("1-4 Deciles")) (Decile5OutputTax, label("5th Decile")) 
         (Decile6OutputTax, label("6th Decile")) (Decile7OutputTax, label("7th Decile")) 
		 (Decile8OutputTax, label("8th Decile")) (Decile9OutputTax, label("9th Decile")) 
		 (TopOutputTax, label("10th Decile")) , drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
...
coefplot (Decile1MoneyDeposited, label("1-4 Deciles")) (Decile5MoneyDeposited, label("5th Decile")) 
         (Decile6MoneyDeposited, label("6th Decile")) (Decile7MoneyDeposited, label("7th Decile")) 
		 (Decile8MoneyDeposited, label("8th Decile")) (Decile9MoneyDeposited, label("9th Decile")) 
		, drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
	     title("Coefficient for MoneyDeposited") 
	     note( "Coefficient in million rupees." "Number of retailers is 32979 and number of wholesalers is 19515");
...
#delimit ;
coefplot (Decile1Interstate, label("1-4 Deciles")) (Decile5Interstate, label("5th Decile")) 
         (Decile6Interstate, label("6th Decile")) (Decile7Interstate, label("7th Decile")) 
		 (Decile8Interstate, label("8th Decile")) (Decile9Interstate, label("9th Decile")) 
		 (TopInterstate, label("10th Decile")), drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
...
#delimit ;
coefplot (Decile1Diff, label("1-4 Deciles")) (Decile5Diff, label("5th Decile")) 
         (Decile6Diff, label("6th Decile")) (Decile7Diff, label("7th Decile")) 
		 (Decile8Diff, label("8th Decile")) (Decile9Diff, label("9th Decile")) 
		 (TopDiff, label("10th Decile")), drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
	     title("Coefficient for Difference measure") 
	     note( "Coefficient in million rupees. Calculate difference measure by subtracting input credits from output tax." "Number of retailers is 32979 and number of wholesalers is 19515");

		 
#delimit ;
coefplot (Decile1lDiff, label("1-4 Deciles")) (Decile5lDiff, label("5th Decile")) 
         (Decile6lDiff, label("6th Decile")) (Decile7lDiff, label("7th Decile")) 
		 (Decile8lDiff, label("8th Decile")) (Decile9lDiff, label("9th Decile")) 
		 (ToplDiff, label("10th Decile")), drop(_cons Post iTaxYear2 iTaxYear4 iTaxYear5) vertical yline(0) 
	     graphregion(color(white))
	     title("Coefficient for Log Difference measure") 
	     note( "Coefficient in million rupees. Calculate difference measure by subtracting input credits from output tax." "Number of retailers is 32979 and number of wholesalers is 19515");
