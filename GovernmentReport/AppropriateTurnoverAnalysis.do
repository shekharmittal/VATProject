/*
What is the appropriate turnover at which the tax collection should be mandated
In this analysis we will draw a graph for MoneyDeposited Vs Turnover
*/

//We first collate data at the annual firm level
...
gen Crore1=0
replace Crore1=1 if TurnoverGross>10000000

gen Crore2=0
replace Crore2=1 if TurnoverGross>20000000

gen lac50=0
replace lac50=1 if TurnoverGross>5000000

gen Crore5=0
replace Crore5=1 if TurnoverGross>50000000
...
preserve
collapse (count) Number=dummy (mean) PositiveContribution (sum) MoneyDeposited, by(TaxYear)
twoway (connected MoneyDeposited TaxYear) (connected Number TaxYear, yaxis(2))
restore



preserve
collapse (count) Number=dummy (mean) PositiveContribution (sum) MoneyDeposited, by(TaxYear Crore1)
twoway (connected MoneyDeposited TaxYear if Crore1==1) (connected Number TaxYear if Crore1==1, yaxis(2))
restore
...
table TaxYear, contents(sum dummy)
table TaxYear, contents(mean PositiveContribution)
table TaxYear, contents(sum MoneyDeposited)
table TaxYear, contents(mean MoneyDeposited)

table TaxYear if lac50==1, contents(sum MoneyDeposited)
table TaxYear if lac50==1, contents(sum dummy )
table TaxYear if lac50==1, contents(mean PositiveContribution)
table TaxYear if lac50==1, contents(mean MoneyDeposited)

table TaxYear if Crore1==1, contents(mean MoneyDeposited)
table TaxYear if Crore1==1, contents(sum MoneyDeposited)
table TaxYear if Crore1==1, contents(mean PositiveContribution)
table TaxYear if Crore1==1, contents(sum dummy)

table TaxYear if Crore2==1, contents(mean MoneyDeposited)
table TaxYear if Crore2==1, contents(sum MoneyDeposited)
table TaxYear if Crore2==1, contents(mean PositiveContribution)
table TaxYear if Crore2==1, contents(sum dummy)

table TaxYear if Crore5==1, contents(mean MoneyDeposited)
table TaxYear if Crore5==1, contents(sum MoneyDeposited)
table TaxYear if Crore5==1, contents(mean PositiveContribution)
table TaxYear if Crore5==1, contents(sum dummy)
